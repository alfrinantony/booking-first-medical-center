...
export async function GET(request: Request) {
    ...
    const assignments = HRShiftStore.getAssignments({ date, startDate, endDate, branchId, employeeId, status });

    // Enrich with employee names
    const enriched = assignments.map(a => {
        const emp = HRStore.getById(a.employeeId);
        return {
            ...a,
    ...
    if (searchParams.get('summary') === 'true') {
        ...
        const summary = HRShiftStore.getDaySummary(summaryDate);
...
export async function POST(request: Request) {
    try {
        ...
        if (action === 'generate-clinician-shifts') {
            const result = HRShiftStore.generateClinicianShifts(data.date, data.branchId);
            return NextResponse.json(result);
        }

        // Update existing assignment
        if (action === 'update' && data.id) {
            const updated = HRShiftStore.updateAssignment(data.id, data);
        ...
        const assignment = HRShiftStore.assignShift(data);
...
export async function DELETE(request: Request) {
    ...
    const deleted = HRShiftStore.deleteAssignment(id);
