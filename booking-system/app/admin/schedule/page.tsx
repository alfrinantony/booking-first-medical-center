...
export default function SchedulePage() {
    ...
    const fetchSchedule = useCallback(async () => {
        ...
        setIsLoading(true);
        try {
            const res = await fetch(`/api/admin/schedule?doctorId=${encodeURIComponent(selectedDoctorId)}&date=${startDate}&clinicId=${encodeURIComponent(selectedClinicId)}&otherBranches=true&editMode=true`);
            if (res.ok) {
                const data = await res.json();
