...
export default function PayrollPage() {
    ...
    const loadPayroll = useCallback(async () => {
        setLoading(true);
        try {
            const params = new URLSearchParams({ month: String(selectedMonth + 1), year: String(selectedYear) });
    ...
    const totals = payroll.reduce(
        (acc, r) => ({
            ...
            gratuity: acc.gratuity + r.gratuityAccrued,
            leaveEncash: acc.leaveEncash + r.leaveEncashment,
        }),
        { grossSalary: 0, gratuity: 0, leaveEncash: 0 }
    );
                ...
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                    <div>
                        <h1 className="text-3xl font-bold text-gray-900 dark:text-white flex items-center gap-3">
                            <Calculator className="w-8 h-8 text-indigo-600" /> Payroll Overview
                        </h1>
                        <p className="text-gray-600 dark:text-gray-400 mt-1">Salary, gratuity, and leave summary for all active employees</p>
                    </div>
                    {/* Month Selector */}
